...
ui_choose() {
  ...
  if have_gum; then
    ...
    local chosen_label
    chosen_label="$(gum choose --header "$header" --timeout "${PROMPT_TIMEOUT_SECS}s" -- "${labels[@]}")" || return 130
...
prompt_and_handle_blockers() {
  ...
  if [[ "$UNATTENDED" == true || "$STDIN_IS_TTY" != true ]]; then
    # Nobody is here to answer. Proceed rather than abort: the sleep
    # itself is now verified and retried, so a blocker that survives
    # shows up as a logged, reported failure instead of a silent
    # refusal — and aborting here would guarantee the machine stays
    # awake, which is the worse of the two outcomes.
    print_warn "${#PREFLIGHT_BLOCKERS[@]} blocker(s) detected, no one to prompt — proceeding; sleep will be verified and retried."
    log_event "BLOCKERS_UNATTENDED_PROCEED count=${#PREFLIGHT_BLOCKERS[@]}"
    return 0
  fi
...
HOOK_MATCH_JQ='def gn_entry:
    ([ (.hooks // [])[] | (.command // "") ] | any(test("goodnight-hook|goodnight/busy")))
    or (._managed_by == "goodnight");'
...
hooks_health() {
  ...
  counts="$(jq -r "${HOOK_MATCH_JQ}"'
    ([ (.hooks.UserPromptSubmit // [])[] | select(gn_entry) ] | length),
    ([ (.hooks.Stop            // [])[] | select(gn_entry) ] | length)
  ...
  if ((ups > 0 && stop > 0)); then
    echo ok
  elif ((ups > 0 || stop > 0)); then
    echo partial
  else
    echo missing
  fi
...
hooks_need_retag() {
  ...
  jq -e "${HOOK_MATCH_JQ}"'
    [ ((.hooks.UserPromptSubmit // []) + (.hooks.Stop // []) + (.hooks.SessionEnd // []))[]
      | select(gn_entry) | select(._managed_by != "goodnight") ] | length > 0
...
repair_claude_hooks() {
  ...
  if hooks_need_retag; then
    ...
    tmp="$(mktemp)" || return 1
    if jq "${HOOK_MATCH_JQ}"'
      def retag: if gn_entry then (._managed_by = "goodnight") else . end;
      .hooks = (.hooks // {})
...
install_claude_hooks() {
  ...
  # ._managed_by tag (gn_entry). Matching on the tag alone is what would
  ...
  if ! jq --arg prompt_cmd "$prompt_cmd" --arg stop_cmd "$stop_cmd" --arg end_cmd "$end_cmd" \
    "${HOOK_MATCH_JQ}"'
    .hooks = (.hooks // {})
    | .hooks.UserPromptSubmit = [ (.hooks.UserPromptSubmit // [])[] | select(gn_entry | not) ]
    | .hooks.Stop             = [ (.hooks.Stop             // [])[] | select(gn_entry | not) ]
    | .hooks.SessionEnd       = [ (.hooks.SessionEnd       // [])[] | select(gn_entry | not) ]
...
smart_watch_loop() {
  local idle_start=0 now busy recent_write
  local frames=("⠋" "⠙" "⠹" "⠸" "⠼" "⠴" "⠦" "⠧" "⠇" "⠏")
  local tick=0 poll
  ...
  while true; do
    ...
    if [[ "$busy" == "0" && "$recent_write" == false ]]; then
      ((idle_start == 0)) && idle_start=$now
      idle_for=$((now - idle_start))
      if ((idle_for >= SMART_IDLE_SECONDS)); then
        clear_line
        print_ok "All agents idle for $(elapsed_label "$SMART_IDLE_SECONDS") — proceeding to sleep."
        log_event "SMART_IDLE_REACHED waited=${elapsed}s"
        return 0
      fi
...
if [[ "$DOCTOR_MODE" == true ]]; then
  ...
  case "$dr_health" in
    ok)
      print_ok "Hooks healthy — UserPromptSubmit and Stop both present."
      ;;
    partial)
      print_error "Hooks PARTIAL — only one of UserPromptSubmit/Stop is installed."
  ...
  if command -v jq >/dev/null 2>&1 && [[ -f "$CLAUDE_SETTINGS_FILE" ]]; then
    for dr_evt in UserPromptSubmit Stop SessionEnd; do
      dr_n="$(jq -r "${HOOK_MATCH_JQ}"' [ (.hooks."'"$dr_evt"'" // [])[] | select(gn_entry) ] | length' \
