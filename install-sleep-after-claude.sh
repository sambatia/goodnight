...
AGENT_CPU_BUSY_PCT="${SAC_AGENT_CPU_BUSY_PCT:-20}"
CPU_GUARD=true
...
if [[ "$LOG_SUMMARY" == true ]]; then
  ...
  fi
  {
    ...
    echo "| Category | Count |"
    echo "|---|---|"
    # Event vocabulary must track the one log_event actually emits.
    # These were left on the pre-rewrite names, so the summary reported
    # zero for every event the current code produces — a report that is
    # confidently wrong is worse than no report.
    for pat in SMART_WATCH_START SMART_IDLE_REACHED SMART_TIMEOUT SLEPT_EXTERNALLY \
      SLEEP_ATTEMPT SLEEP_CONFIRMED SLEEP_REFUSED SLEEP_FAILED \
      HOOKS_DEGRADED HOOKS_RETAGGED HOOKS_REINSTALLED MARKERS_REAPED \
      PREFLIGHT_BLOCKERS BLOCKERS_UNATTENDED_PROCEED PREFLIGHT_SCAN_FAILED \
      POWER_GATE_WAITING AUTO_CAFFEINATE_STARTED CAFFEINATE_RELEASED \
      WATCH_START CLAUDE_FINISHED CANCELLED TIMEOUT PID_REUSED; do
...
if [[ "${SMART_WATCH_DONE:-false}" != true ]]; then
  ...
  while [[ "${SMART_WATCH_DONE:-false}" != true ]] && kill -0 "$TARGET_PID" 2>/dev/null; do
    ...
    if [[ -n "$PID_ENTRY_SLEEP_STAMP" ]] && ((TICK_COUNT % 50 == 0)); then
      NOW_SLEEP_STAMP="$(sleep_stamp)"
