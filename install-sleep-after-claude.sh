...
# Every PID in an agent's process tree: the agent itself plus all
# descendants.
#
# The work an agent does is mostly done by its children. A background
# shell, a Monitor, a test run, a build — Claude Code and Codex spawn
# these as child processes named `zsh`, `node`, `npm`, not `claude`.
# `ps -o time=` reports a process's own CPU and never its children's, so
# measuring only the named agent processes misses precisely the work
# that keeps a machine busy after a turn has ended and the busy marker
# has already been cleared.
#
# Measured on a live machine: `pgrep -x claude` found 1 process where
# the real tree held 22, with 293 CPU-seconds outside the guard's view.
#
# Depth-capped so a pathological parent/child cycle cannot spin here.
agent_process_tree() {
  local all frontier next name depth=0
  all="$(
  ...
  while [[ -n "${frontier// /}" ]] && ((depth < 12)); do
    next="$(pgrep -P "$(printf '%s' "$frontier" | tr -s ' ' ',' | sed 's/^,//;s/,$//')" 2>/dev/null | tr '\n' ' ')"
    [[ -z "${next// /}" ]] && break
    all="$all $next"
    frontier="$next"
    depth=$((depth + 1))
